...
int multiGeneCircuit_RK_deterministic(IntegerMatrix gene_interaction, NumericVector threshold_gene,
                                      ...
                                      int output_precision, int INITIAL_CONDITIONS)

{
    ...
    std::fstream out_0("./results/sRACIPE_RK_g"+std::to_string(number_gene)+"_parameters.txt",std::ios::app);
    if(!out_0) {     Rcout << "Cannot open output file for writing parameters.\n";  }
    
    std::fstream out_ic("./results/sRACIPE_RK_g"+std::to_string(number_gene)+"_IC.txt",std::ios::app);
    if(!out_ic) {     Rcout << "Cannot open output file for writing parameters.\n";  }
    
    std::fstream out_1("./results/sRACIPE_g"+std::to_string(number_gene)+"_output.txt",std::ios::app);
