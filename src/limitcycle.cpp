        ...
        if(period>0){
            ...
            for (j=0;j<nIC;j++){
                ...
                for(k=0;k<period+2;k++){
                    tmp_norm=sum_delta(EXP_arr[j],LC_exp_arr[k],number_gene);
        ...
        while(countStep<LCSimSteps){
            ...
            if(moving_uphill){ //while moving uphill:
                ...
                if (curr_dist>prev_dist){
                    ...
                    min_idx_arr[countMinExp]=count_exp;
...
int cal_period(const int &number_gene,
    ...
    for (int i=countMinExp-2;i>=0;i--){
       ...
       if(diffExp<=SamePointProximity){
          countSameMinDist++;
          periodT[countPeriod]=minIdxSrr[idx_lastPeriod]-minIdxArr[i];
...
void cal_fX(const int &number_gene,
    ...
    for (int i=0;i<number_gene;++i) {
        fX_arr[i]=g_gene[i]; 
        fX2_arr[i]=k_gene[i]*expression_gene[i]; 
    }
